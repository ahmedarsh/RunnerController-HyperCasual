...
public partial class MTouchInput : MonoBehaviour
{
	[HideInInspector]
	...
	private MTouch[] _touchCache;
	private List<MTouch> _liveTouches = new List<MTouch>( 2 );
	...
	private static MTouchInput _instance = null;
	public static MTouchInput instance
	{
		get
		{
			if( !_instance )
			{
				...
				_instance = FindObjectOfType( typeof( MTouchInput ) ) as MTouchInput;

				// nope, create a new one
				if( !_instance )
				{
					var obj = new GameObject( "TouchKit" );
					_instance = obj.AddComponent<MTouchInput>();
	...
	private void Awake()
	{
		...
		_touchCache = new MTouch[maxTouchesToProcess];
		for( int i = 0; i < maxTouchesToProcess; i++ )
			_touchCache[i] = new MTouch( i );
	}
